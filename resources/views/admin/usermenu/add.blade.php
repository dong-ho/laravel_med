...
<div class="col-xl-12">
    ...
    <div class="card mb-4">
        <div class="card-header d-flex align-items-center justify-content-between">
            <h5 class="mb-0">사용자 메뉴 추가</h5>
            <small class="text-muted float-end"></small>
        </div>
        <div class="card-body">
            <form id="formMenu" method="POST" action="{{ route('admin.usermenu.store') }}" >
                @csrf
                <dif class="row">
                    <div class="mb-3 col-md-6">
                        <label for="name" class="form-label">메뉴명</label>
                        <input class="form-control" type="text" id="name" name="name" placeholder="메뉴명" value="{{ old('name') }}"/>
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <select id="parent_id" name="parent_id" class="form-select">
                            ...
                            @foreach($parentLists as $parentList)
                                <option value="{{$parentList->id}}" {{$parentList->id===old('parent_id')?'selected':''}}>{{$parentList->name}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <input class="form-control" type="text" id="url" name="url" placeholder="라우트명" value="{{ old('url') }}"/>
                        <span class="error">{{ $errors->first('url') }}</span>
                    </div>
                    <div class="mb-3 col-md-6">
                        <label for="icon" class="form-label">아이콘</label>
                        <input class="form-control" type="text" id="icon" name="icon" placeholder="아이콘" value="{{ old('icon') }}"/>
                    ...
                    <div class="mb-3 col-md-6">
                        ...
                        <select id="sort_order" name="sort_order" class="form-select">
                            @for($i=1;$i<10;$i++)
                                <option value="{{$i}}" {{$i==old('sort_order')?'selected':''}}>{{$i}}</option>
                            @endfor
                        </select>
                ...
                <div class="mt-2">
                    ...
                    <a href="{{ route('admin.usermenu') }}"  class="btn btn-outline-secondary">
