<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <ul class="menu-inner py-1">
        ...
        @foreach($menus as $menu)
            @if(empty($menu->parent_id) && $menu->children->isNotEmpty())
                <li class="menu-header small text-uppercase">
                    <span class="menu-header-text">{{ $menu->name }}</span>
                </li>
            @endif
            <li class="menu-item {{ $menu->active ?  'open active' : '' }}">
                <a href="{{ $menu->url? url(str_replace('.','/',$menu->url)) :'javascript:void(0);' }}" class="menu-link {{ $menu->children->isNotEmpty()? 'menu-toggle':''}} on">
                    ...
                    <ul class="menu-sub">
                        @foreach($menu->children as $child)
                            <li class="menu-item {{ $child->active ?  'active' : '' }}">
                                <a href="{{ $child->url? url(str_replace('.','/',$child->url)) :'javascript:void(0);' }}" class="menu-link">
